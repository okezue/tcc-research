...
for r in 16 32 64 128; do
  for b in 1e-3 1e-2 1e-1 3e-1; do
    dispatch run_one $r $b 0 0 0
  done
done
wait

# Stage 2B: gamma x sigma at top (r,b)
echo "=== Stage 2B: gamma x sigma sweep ==="
for r in 32 64; do
  for b in 1e-2 1e-1; do
    for g in 0.1 0.3; do
      for s in 0.1 0.2 0.5; do
...
# Stage 2C: 3 seeds on the most promising config (defaults: r=64, b=1e-1, g=0.1, s=0.2)
echo "=== Stage 2C: 3 seeds ==="
for cfg in "64 1e-1 0.1 0.2" "32 1e-2 0.3 0.5"; do
  for sd in 1 2; do
    dispatch run_one $cfg $sd
  done
done
wait
